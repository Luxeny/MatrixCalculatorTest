...
public class MathMatrix : ICloneable, IComparable<MathMatrix>
{
    private int[,] _matrix;
    private int _size;

    public MathMatrix(int size)
    {
        _size = size;
    ...
    public static bool operator >(MathMatrix firstMatrix, MathMatrix secondMatrix)
    {
    ...
    public static explicit operator int(MathMatrix matrix)
    {
        return matrix.Determinant();
    }

    public static bool operator true(MathMatrix matrix)
    {
        return matrix.Determinant() != 0;
    }

    public static bool operator false(MathMatrix matrix)
    {
        return matrix.Determinant() == 0;
    }
    ...
    public int Determinant()
    {
        ...
        for (int rowDeterminant = 0; rowDeterminant < _size; ++rowDeterminant)
        {
            ...
            determinant += (int)Math.Pow(-1, rowDeterminant) * _matrix[0, rowDeterminant] * new MathMatrix(subMatrix).Determinant();
        }
        return determinant;
    }
        ...
        for (int row = 0; row < _size; ++row)
        {
            for (int column = 0; column < _size; ++column)
            {
                ...
                for (int rowDeterminant = 0, subRow = 0; rowDeterminant < _size; ++rowDeterminant)
                {
                    ...
                    {
                        if (columnDeterminant == column) continue;
                        subMatrix[subRow, subColumn] = _matrix[rowDeterminant, columnDeterminant];
                        subColumn++;
                    }
                    subRow++;
                }
                inverse[column, row] = (int)Math.Pow(-1, row + column) * new MathMatrix(subMatrix).Determinant() / determinant;
            }
        }
        return new MathMatrix(inverse);
    }

    public MathMatrix DeepClone()
    {
        return new MathMatrix((int[,])_matrix.Clone());
    }
    ...
    public int CompareTo(MathMatrix other)
    {
